...
ES_t Timer0_enuAsysncDelay(int copy_ms){
	if(TIMER0_MODE==TIMER0_Normal){ //(TIMER0_PRESCALER==TIMER0_SCALER_1||TIMER0_PRESCALER==TIMER0_SCALER_8)&&
	    float local_ovf_t= (float)TIMER0_OVF_COUNTS * ((float)TIMER0_PRESCALER/TIMER0_F_CPU);
		float ovf_num= copy_ms/local_ovf_t;
		if(ovf_num-(int)ovf_num!=0.0){
			//int int_ovf_num=(int)ovf_num+1;
			int preload=TIMER0_OVF_COUNTS-((ovf_num-(int)ovf_num)*256);
			ovf_num=ceil(ovf_num);
		...
		if(ovf_num-(int)ovf_num!=0.0){
		//int int_ovf_num=(int)ovf_num+1;
		int preload=TIMER0_OVF_COUNTS-((ovf_num-(int)ovf_num)*TIMER0_OVF_COUNTS);
		ovf_num=(int)ovf_num+1;
		TIMER0_VIOD_SET_PRELOAD(preload);
	  }
	}
	else{
